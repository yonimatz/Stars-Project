...
def Luminosity(R_surface,T_surface):
    return 4*pi*sigma*(R_surface**2)*(T_surface**4)

def LDiffFinder(L_expected,L_surface):
    if L_surface!=0:
        Ldiff = (L_surface-L_expected)/(np.sqrt(L_expected*L_surface))
    else:
        Ldiff = None 
    # sometime L_surf = 0 for some reason, but if it is, return none ans start an error catch sequence.
    return Ldiff 

def RunRK45ToGetL(centralRho,inputs,centralTemp):
    
    values = CreateInitialParameters(inputs['r_0'], centralRho , centralTemp)
...
numstars = 2
T_c = np.linspace(1e6,1e7,numstars)
T_c = [8.23544e+06,2e+07]
Stars = np.zeros((2,numstars))       
i = 0    
while i <= numstars-1:
    
    vals, r = BisectionRootSolver(300,500000,T_c[i],inputs, 0.01)
    try:
        if vals == None:# if no root was foun, skip this star
            i+=1
            continue
    except:
        pass
    ...
    print("!!Star found!!!")
    print("R Surface: ", R_surface)
    print("L Surface: ", L_surface)
    print("T Surface: ", T_surface)
    print("Mass: ", Star_Mass)
    print('\n')
...
#                print('Decreasing rho_c to: '+ str(values[0]) + '\n')
#            else:
#                values[0] = values[0]*1.1 # # If error is too large, make rho_c smaller
#                print('Increasing rho_c to: '+ str(values[0]) + '\n')
#
