...
filename = 'V5_0_data_x08_y02.csv'
title = 'L (row 1), T (row 2), M (row 3), R (row 4), T_c (row 5), rho_c (row 6), tau_0 = 0, X = 0.8, Y = 0.2'

# rho_c = 58556
rho_c = 300000
rho_c_list = [rho_c]

T_c = np.linspace(1e6,1e7,25)
numstars = len(T_c)
...
while i <= numstars - 1:
    rho_c = rho_c_list[i]
    vals, r = BisectionRootSolver(rho_c*0.01, rho_c*100, T_c[i], inputs, 0.1) # The min and max densities, accuracy we are looking at can be varied
