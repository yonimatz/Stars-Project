...
while i <= numstars-1:
    ...
    while (match == False):
        ...
        tau_surface = np.max(vals[4,:])-(2/3)
        idx = np.abs(vals[4] - tau_surface).argmin()
        R_surface = r[idx]
        L_surface = vals[3,idx]
        T_surface = vals[1,idx]

        L_error = (L_surface-(4*pi*sigma*(R_surface**2)*(T_surface**4)))/(np.sqrt(4*pi*sigma*(R_surface**2)*(T_surface**4)*L_surface))
        print(L_error)


        if L_error < 0.1 and L_error > -0.1:
            match = True
            Stars[0,i] = L_surface
            Stars[1,i] = T_surface
            print("R Surface: " + str(R_surface))
            i +=1
        else:
            if L_error > 0:
                values[0] = values[0]*0.9
            else:
